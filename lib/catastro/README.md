...
- Vía oficial sin direcciones INSPIRE: el WFS responde `302 → /OVCError.aspx` (HTTP 404). El cliente HTTP detecta esa redirección y la marca con `CATASTRO_OVCERROR_STATUS` (502 sintético) para no confundirla con un 404 real. `descubrirNumerosOficiales` lo trata como **0 portales** (no error); verificado en Godelleta (`DS DISEMINADO P 1`, `UR EL BOSQUE 1`). Un 5xx o un fallo de red siguen siendo error externo.
- `GetADByPostalCode` responde con la misma redirección tanto si el CP no existe como si el servicio falla para un CP real (visto con Godelleta 46388). Como no se distinguen, `prepararZona` no da el CP por vacío: recorre el callejero completo y aplica el filtro de CP finca a finca.
- Timeouts: 20 s por defecto; el callejero completo de un municipio y los listados INSPIRE por CP usan `CATALOGO_TIMEOUT_MS` (45 s), porque en capitales grandes (Madrid, 1,6 MB) Catastro tarda ~30 s.
- Tests: `npm test` corre con `CATASTRO_SKIP_LIVE=1` (sin red). `npm run test:live` ejecuta también los que llaman a Catastro de verdad.
